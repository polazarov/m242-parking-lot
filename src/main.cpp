...
void timerCallback()
{
    mqtt_publish("parking_garage", "Parking is used too long");
    //Telegram Stuff
}
...
void mqtt_callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (String(topic) == "parking_garage")
  {
    if (payloadS == "Free")
    {
      lv_label_set_text(labelValue, "2 Open Parkings");
    } else if (payloadS == "Used") {
      lv_label_set_text(labelValue, "0 Open Parkings");
    }
  }
}
